...
class StudentIdCards extends Page {
    ...
    public function form(Form $form): Form {
        return $form->schema([
            Forms\Components\Radio::make('generate_for')->label('Generate For')
                ->options(['section'=>'One Class/Section','all'=>'All Active Students'])
                ->default('section')->live()->inline(),
            Forms\Components\Select::make('section_id')->label('Class & Section')
                ...
                ->searchable()->visible(fn(Forms\Get $g)=>$g('generate_for')==='section'),
            Forms\Components\Select::make('academic_year_id')->label('Academic Year')
    ...
    protected function getFormActions(): array {
        return [
            Action::make('generate')->label('Generate ID Cards')->submit('generate')->icon('heroicon-o-identification'),
            Action::make('print')->label('Print All')->action(fn()=>null)->extraAttributes(['onclick'=>'window.print()'])->color('gray')->icon('heroicon-o-printer'),
        ];
    }
}