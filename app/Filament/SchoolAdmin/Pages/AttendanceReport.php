...
class AttendanceReport extends Page {
    ...
    public function mount(): void {
        ...
        $this->form->fill(['section_id'=>null,'month'=>now()->format('Y-m'),'report_type'=>'monthly_summary']);
    }

    public function form(Form $form): Form {
        return $form->schema([
            Forms\Components\Select::make('report_type')->label('Report Type')
                ->options(['monthly_summary'=>'Monthly Summary per Student','low_attendance'=>'Low Attendance List','daily'=>'Daily Attendance'])
                ->required()->live()->default('monthly_summary'),
            Forms\Components\Select::make('section_id')->label('Class & Section')
                ...
                ->required()->searchable(),
            Forms\Components\TextInput::make('month')->label('Month')->type('month')->default(now()->format('Y-m'))
                ->visible(fn(Forms\Get $g)=>in_array($g('report_type'),['monthly_summary','low_attendance'])),
            Forms\Components\DatePicker::make('date')->label('Date')->default(today())
                ->visible(fn(Forms\Get $g)=>$g('report_type')==='daily'),
