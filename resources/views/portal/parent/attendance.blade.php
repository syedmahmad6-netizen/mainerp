...
<div class="flex items-center gap-3 mb-5">
  ...
  <div>
    <h2 class="text-lg font-bold text-gray-800">Attendance — {{ $student->user->name }}</h2>
    <p class="text-xs text-gray-500">{{ $student->section?->schoolClass?->name }} – Section {{ $student->section?->name }}</p>
  </div>
</div>
...
<div class="bg-white rounded-xl p-4 mb-5 shadow-sm border border-gray-100">
  ...
  <div class="bg-gray-100 rounded-full h-3">
    <div class="h-3 rounded-full transition-all {{ $pct>=75?'bg-emerald-500':'bg-red-500' }}" style="width:{{ min($pct,100) }}%"></div>
  </div>
  @if($pct<75)<p class="text-xs text-red-500 mt-2">⚠️ Below 75% threshold. Please ensure regular attendance.</p>@endif
</div>
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="grid grid-cols-7 gap-1">
    ...
    @for($day=1;$day<=$to->day;$day++)
      ...
      <div class="aspect-square flex items-center justify-center rounded-lg text-xs {{ $col }}" title="{{ $rec?ucfirst($rec->status):($isWe?'Weekend':'') }}">{{ $day }}</div>
    @endfor
  </div>
