...
<title>@yield('title','Parent Portal') — {{ tenant()->getSchool()->name }}</title>
<script src="https://cdn.tailwindcss.com"></script>
@php $school=tenant()->getSchool(); $pc=$school->setting('primary_color','#059669'); @endphp
<style>:root{--cp:{{ $pc }}}.bg-cp{background-color:var(--cp)}.text-cp{color:var(--cp)}.ring-cp:focus{outline-color:var(--cp)}</style>
</head>
  ...
  <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-xs opacity-80 mt-0.5">Parent Portal</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
...
<div class="max-w-5xl mx-auto md:flex md:gap-6 md:p-6">
  <nav class="hidden md:block w-52 shrink-0">
    <div class="bg-white rounded-2xl shadow-sm p-3 sticky top-20">
      @foreach([['route'=>'parent.dashboard','label'=>'Dashboard','icon'=>'🏠'],['route'=>'parent.announcements','label'=>'Announcements','icon'=>'📢']] as $item)
...
<nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-30 flex">
  @foreach([['route'=>'parent.dashboard','label'=>'Home','icon'=>'🏠'],['route'=>'parent.announcements','label'=>'Notices','icon'=>'📢']] as $item)
