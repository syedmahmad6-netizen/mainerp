...
<div class="flex items-center gap-3 mb-5">
  ...
  <h2 class="text-lg font-bold text-gray-800">Timetable — {{ $student->user->name }}</h2>
</div>
@if(empty($grid))
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-xs border-collapse min-w-max">
      <thead class="bg-gray-50"><tr>
        <th class="border border-gray-100 px-3 py-2 text-left text-gray-600 font-medium">Period</th>
      ...
      <tbody>
        ...
        <tr class="{{ $slot->is_break?'bg-amber-50':'hover:bg-gray-50' }}">
          <td class="border border-gray-100 px-3 py-2">
            <div class="font-medium text-gray-700 text-xs">{{ $slot->name }}</div>
            <div class="text-gray-400 text-xs">{{ \Carbon\Carbon::parse($slot->start_time)->format('h:i A') }}</div>
          </td>
          @foreach($days as $dn=>$d)
            @php $e=$grid[$dn][$slot->id]??null; @endphp
            <td class="border border-gray-100 px-2 py-1.5 text-center">
              @if($slot->is_break)<span class="text-amber-500 text-xs">Break</span>
              @elseif($e)
                <div class="rounded px-1.5 py-1 text-left text-xs" style="background:{{ $e->subject?->color??'#e0f2fe' }}22;border-left:2px solid {{ $e->subject?->color??'#0ea5e9' }}">
                  <div class="font-semibold text-gray-800">{{ $e->subject?->name }}</div>
                  <div class="text-gray-500">{{ $e->teacher?->name }}</div>
                </div>
